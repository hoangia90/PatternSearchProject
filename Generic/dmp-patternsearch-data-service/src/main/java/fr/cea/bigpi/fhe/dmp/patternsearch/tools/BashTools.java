package fr.cea.bigpi.fhe.dmp.patternsearch.tools;
...
public class BashTools {
	...
	public static boolean Exec(boolean isShowLog, boolean isWithBash, String script1, String script2, String script3,
			String script4, String script5, String script6, String script7, String script8, String script9) {
		if (isShowLog)
			log.info("Starting exec inputs paramaters");
		Process childExec = null;
		String bash = "/bin/bash";

		log.info(
				String.format("%s %s %s %s %s %s %s %s %s %s %s\n", "Start Exec String Params:", isWithBash ? bash : "",
						script1, script2, script3, script4, script5, script6, script7, script8, script9));
		...
		try {
			if (script2.length() == 0)
				...
				if (isWithBash)
					childExec = new ProcessBuilder(bash, script1, script2, script3, script4, script5, script6, script7)
							.start();
				else
					childExec = new ProcessBuilder(script1, script2, script3, script4, script5, script6, script7)
							.start();
			} else if (script8.length() == 0) {
				if (isWithBash)
					childExec = new ProcessBuilder(bash, script1, script2, script3, script4, script5, script6, script7,
			...
			if (isShowLog) {
				BufferedReader output = new BufferedReader(new InputStreamReader(childExec.getInputStream()));
				String line = null;
				while ((line = output.readLine()) != null)
					log.info("Exec output > " + line);
			}
			log.info(String.format("%s %d %s %s %s %s %s %s %s %s %s %s %s\n", "End with code :", code,
					" Exec String Params:", isWithBash ? bash : "", script1, script2, script3, script4, script5,
	...
	public static boolean Exec(boolean isWithBash, String script1, String script2, String script3, String script4,
			String script5, String script6, String script7, String script8, String script9) {
